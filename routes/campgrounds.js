...
router.post("/campgrounds",middleware.isLoggedIn, upload.single("image"),(req,res)=>{ 
    // console.log(req.body.campground.start,req.body.campground.start)
    // console.log(req.body.campground);
    // if(req.file){
    //   console.log("abe yaar fir se")
    // }
  ...
  Campground.findById(req.params.id)
    ...
    .exec(function(err, foundCampground) {
      ...
      if (ratingsArray.length === 0) {
        ...
        // console.log("foundcampground " + foundCampground.comments.length);
        // console.log("milgya " + ratings / foundCampground.comments.length);
      }
          ...
          if(req.file){
            Campground.findById(req.params.id,(err,campground)=>{
              ...
              cloudinary.v2.uploader.destroy(campground.imageId,(err,result)=>{
                ...
                // upload a new one
                cloudinary.v2.uploader.upload(req.file.path,{width:1500,height:1000,crop:"scale"},(err,result)=>{
                  // console.log("ha re ab to ho ja " +result.secure_url)
                  if(err){
                    req.flash('error',err.message);
                    return res.redirect("back");
                  }
